...
class VerifyBeneficiary extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    PressYes = (item1, item2) => {
        ...
        this.toggleModal();
        this.setState({
            beneficiaryAccType: item1.BAADHAR_REFNO,
            benificiaryAcc: item1.BAADHAR_REFNO,
            benificiaryAccTitle: item1.BNFID,
            isClick: item2,
        });
        this.SendOtpRequest(item1.BNFID, '4');
    }
    ...
    onSelectAccount = (value, title, acmastcode, acName) => {
        ...
        if (acmastcode === null || acmastcode === undefined || acmastcode.trim() === '') {
            ...
            this.setState({ accType: value, fromAccAcmastCode: acmastcode });
    ...
    handleSearch = (searchTerm) => {
        ...
        const filteredData = this.state.myJsonArrayforBeneficiaryList
            .filter((item) => {
                const lowerCaseName = item.BAADHAR_REFNO.toLowerCase();
    ...
    GetBeneficiaryList = async (acmastcode, acNo) => {
        ...
        if (isConnected) {
            try {
                const Headers = APIUrlConstants.Headers('GETFTLIST');
                const jsonReq = {
    ...
    SendOtpRequest = async (bnfId, NMODE) => {
        const isConnected = await this.checkInternetConnection();
        if (isConnected) {
            try {
                ...
                sendData(
                    ...
                    async (obj, response) => {
                        ...
                        if (finalRes.SUCCESS === 'FALSE') {
                            ...
                            //navigation.navigate(this, 'fundTransferSameBankSuccess', { from: 'Add' })
                        }
                    }
                );
            } catch (e) {
    ...
    SubmitRequest = async () => {
        ...
        if (isConnected) {
            try {
                const Headers = APIUrlConstants.Headers('VERIFTBENFOTP');
                ...
                const jsonReq = {
                    ...
                    secKey: Constants.SecretKey,
                    divId: this.props.DeviceId,
                    bnfId: this.state.benificiaryAccTitle,
                    ...
                    >
                        ...
                        <View style={styles.lableHeader}>
                            <View
                                ...
                                <Text
                                    ...
                                    Verify Your Newly Added Beneficiary.
                                </Text>
                            </View>
                        </View>
                        <View
                            ...
                            <ScrollView>
                                ...
                                <Text
                                    style={[styles.enterOTPText, { color: this.props.textColor, }]}>
                                    {strings.enterOTP}
                                </Text>
                                <View
                                    ...
                                    <View style={styles.resendOtpView}>
                                        {this.state.timer === '00' ? (
                                            <TouchableOpacity
                                                onPress={() => {
                                                    this.setState({ otp: '' });
                                                    this.SendOtpRequest(this.state.benificiaryAccTitle, '4');
                                                }}
                                            >
                                ...
                                <TouchableOpacity
                                    ...
                                    <Text
                                        style={[styles.submitButtonText, { color: this.state.otp_length.length >= 3 ? colors.white : colors.btnDisableTextColor, }]}>
                                        Submit
                                    </Text>
                                </TouchableOpacity>
                            </ScrollView>
                        </View>
                        {
                            DialogBeneficiaryPopup(
                                ...
                                null,
                                'Within Other Verify Beneficiary'
                            )
                        }
...
export default connect(mapStateToProps, mapDispatchToProps)(VerifyBeneficiary);