...
class NeftTransferDetails extends Component {
    constructor(props) {
        super(props);
        this.state = {
            ...
            benfId: this.props.route.params.bnfId,
    ...
    toEdit() {
        navigation.navigate(this, 'NeftTransfer', { from: this.state.callFrom, accList: this.state.accList })
    }
            ...
            try {
                ...
                console.log("");
                    ...
                    async (obj, response) => {
                        ...
                        if (response.Acdtls != null && response.Acdtls != '') {
                            ...
                            if (response.SUCCESS === "FALSE") {
                                ...
                                navigation.navigate(this, 'NeftTransferOtp', { from: this.state.callFrom, fromAcc: this.state.fromAcc, toAcc: this.state.benificiaryAcc, remark: this.state.remark, amount: this.state.amount, fromAccAcmastCode: this.state.fromAccAcmastCode, beneficiaryAccAcmastCode: this.state.beneficiaryAccAcmastCode, fromAccName: this.state.fromAccName, beneficiaryAccName: this.state.beneficiaryAccName, benfId: this.state.benfId, chrg_amt: this.state.chrg_amt, bnfBrCd: this.state.bnfBrCd, neftTrnid: response.NEFT_TRN_ID, trnscnId: response.TRNSCNID, accList: this.state.accList })
                            }
                          }
                    })
            }
    ...
    convertToWords = (amount) => {
        try {
            ...
            const decimalInWords = decimalPart ? ` and ${numberToWords.toWords(Number(decimalPart))} Paise` : '';
            const capitalizedIntegerInWords = integerInWords.charAt(0).toUpperCase() + integerInWords.slice(1);
            ...
            <View style={styles.mainView}>
                <ImageBackground style={styles.imageBackgroundStyle}
                    ...
                    <View style={styles.lableHeaderMainView}>
                        <View style={styles.lableHeaderSubView}>
                            <Text
                                style={styles.ConfirmDetailsTitle}>NEFT</Text>

                            <Text style={styles.ConfirmDetailsDescription}>Fund Transfer
                            </Text>
                        </View>
                    </View>
                        ...
                        <ScrollView showsVerticalScrollIndicator={false}>
                            <View
                                ...
                                <View
                                    ...
                                    <View style={styles.fromAccOneView}>
                                    ...
                                    <View style={styles.toAccMainView}>
                                        <View style={styles.fromAccTwoView}>
                                            <View style={styles.mainView}>
                                                <Text style={[styles.fromAccText, { color: this.props.themeColor }]}>
                                                    To A/c
                            ...
                            <View style={styles.remarkMainView}>
                                <View style={styles.fromAccTwoView}>
                                    <View style={styles.mainView}>
                                        <Text style={[styles.fromAccText, { color: this.props.themeColor }]}>
                                            Remark
                            ...
                            <Text style={[styles.amountInWordsText, { color: this.props.themeColor }]}>
                                Amount in Words:
                            </Text>
                            ...
                            <Text style={styles.servichChargeConToWordsValue}>
                                {"+ " + this.convertToWords((Number(this.state.chrg_amt))) + " Service Charge"}
                            </Text>
                            <View style={styles.termsAndConditionView}>
                                <TouchableOpacity s style={styles.termsAndConditionTouchable}
                                    onPress={() =>
                                        this.setState({ termsAndCondition: true })
                                    }>
                                    <Iicon height={15} width={15} />
                                    <Text
                                        style={[styles.termsAndConditonText, { color: this.props.textColor }]}>Terms & Conditions Apply</Text>
                                </TouchableOpacity>
                                <TouchableOpacity
                                ...
                                <TouchableOpacity
                                    ...
                                    onPress={() => this.toEdit()}>
                                    <Text
                                        style={[styles.editText, { color: this.props.themeColor }]}>Edit</Text></TouchableOpacity>
                            </View>
                        </ScrollView>
                        {
...
export default connect(mapStateToProps, mapDispatchToProps)(NeftTransferDetails);
