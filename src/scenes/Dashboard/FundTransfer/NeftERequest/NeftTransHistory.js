...
class NeftTransHistory extends Component {
  ...
  SearchClick = () => {
    this.state.start_count = 0,
      this.state.end_count = 5,
      // this.state.Transaction.splice(0, this.state.Transaction.length);
      this.state.Transaction = [],
    ...
    if (isConnected) {
      try {
        ...
        const Body =
        {
          ...
          PARA2_TYP: "STR",
          PARA2_VAL: "{\"TXT_GMST_CODE\":\"" + Constants.GMST_CODE + "\",\"TXT_TRANS_TYPE\":\"N\",\"TXTSTARTROW\":\"" + this.state.start_count + "\",\"TXTENDROW\":\"" + this.state.end_count + "\",\"TXT_NAME\":\"" + this.state.searchTerm + "\"}",
          PARA3_TYP: "STR",
        ...
        sendData(this,
          ...
          async (obj, response) => {
            ...
            if (finalRes.SUCCESS === "FALSE") {
              ...
              Snackbar.show({ text: 'No Record Found', duration: Snackbar.LENGTH_SHORT, backgroundColor: 'red' });
        ...
        <ImageBackground style={styles.mainView}
          ...
          <View style={styles.headerView}>
            <View style={styles.transactionHistoryView}>
              <Text style={styles.Heading}>Transaction History (NEFT)</Text>
          ...
          <View style={styles.CurveView}>
            <View style={styles.curveSubView}>
              <CardView
                ...
                <TouchableOpacity style={[styles.SeacrhBg, { backgroundColor: this.props.SecondaryColor, }]}
                  onPress={() => {
                    this.SearchClick()
                  }}>
                ...
                renderItem={({ item }) =>
                  <View>
                    ...
                    <View style={styles.dateViewRow}>
                      <View style={[styles.IconBg, { backgroundColor: item.STATUS === 'C' ? "#eb5757" + '1A' : item.STATUS === 'R' ? "#eb5757" + '1A' : item.STATUS === 'P' || item.STATUS === 'E' ? "#27AE60" + '1A' : item.STATUS === 'A' || item.STATUS === '' ? "#FEBE5C" + '1A' : null }]}>
                        <Withdraw height={16} width={16} color={item.STATUS === 'C' ? "#eb5757" : item.STATUS === 'R' ? "#eb5757" : item.STATUS === 'P' || item.STATUS === 'E' ? "#27AE60" : item.STATUS === 'A' || item.STATUS === '' ? "#FEBE5C" : null} />
                      </View>
                      <View style={styles.rowNameAndNo}>
                        <View style={styles.beneficiaryNameView}>
                          <Text
                            style={styles.FlatHeadingText}
                            numberOfLines={1}>{item.BENF_NAME}</Text>
                          <Text style={styles.FlatListLightText}>A/c No To: {item.TO_ACNO}</Text>
                        </View>
                        <View style={styles.beneficiaryNameView}>
                          <Text style={[styles.FlatHeadingText, { color: '#686868', textAlign: 'right', fontWeight: '600' }]}>₹ {item.AMOUNT}</Text>
...
export default connect(mapStateToProps, mapDispatchToProps)(NeftTransHistory);